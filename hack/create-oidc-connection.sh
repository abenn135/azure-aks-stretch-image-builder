...
SP_PRINCIPAL_ID=$(az identity show --name $AZURE_IDENTITY_NAME --resource-group $RESOURCE_GROUP_NAME --query 'principalId' -o tsv)
az role assignment create --assignee-object-id $SP_PRINCIPAL_ID --assignee-principal-type ServicePrincipal --role "Contributor" --scope "/subscriptions/$AZURE_SUBSCRIPTION_ID/resourceGroups/$RESOURCE_GROUP_NAME"
